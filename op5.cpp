...
void graficarMaximizacion() {
    ...
    // Dibujar líneas de inecuaciones 
...
// Recorre con índice para asignar colores
for (size_t i = 0; i < restriccionesRegistradas.size(); ++i) {
    ...
    for (int x = 0; x < LIMITE_GRAFICO_X; ++x) {
        for (int y = 0; y < LIMITE_GRAFICO_Y; ++y) {
            if (cumpleTodasLasRestricciones(x, y)) {
                ...
                pixel.setFillColor(sf::Color(255, 0, 0, 100)); //color
    ...
    while (window.isOpen()) {
        ...
        for (auto& texto : etiquetasRestricciones)
         window.draw(texto);
        for (auto& etiqueta : etiquetasRestricciones)
        ...
        // Dibujar el punto óptimo como un círculo blanco
        sf::CircleShape puntoOptimo(5.f);
        puntoOptimo.setFillColor(sf::Color::Green); // o el color que prefieras
